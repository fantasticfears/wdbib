...
int main(int argc, char** argv)
{
  ...
  add_cmd->callback([&]() { ProcessAddQIDs(add_qIDs); });
  ...
  list_cmd->callback([&]() { ProcessList(); });

  auto search_cmd =
      app.add_subcommand("search", "Search bibliographical items on Wikidata.");

  auto view_cmd = app.add_subcommand("view", "View a bibliographical item.");
  CommandShowOption show_opt;
  view_cmd->add_flag("-s,--scholia", show_opt.scholia,
                       "show item on Scholia.");
  view_cmd->add_flag("-w,--wikidata", show_opt.wikidata,
                       "show item on Wikidata.");
  string view_string;
  view_cmd->add_option("id", view_string, "id to view");
  view_cmd->callback([&]() { ProcessShowQID(view_string, show_opt); });
